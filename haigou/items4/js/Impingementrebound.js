function CanvasConstructor(idname){
		this.idname = idname;//不带#
		var VELOCITY = 20;//速率
		var PARTICLES = 10;//粒子数量
		
		var mouse = {x:0, y:0};//坐标
		var particles = [];//一个综合的坐标
		...
		if (canvas && canvas.getContext) {
			...
			for( var i = 0; i < PARTICLES; i++ ) {
				particles.push( { 
					...
					// color:"rgba(#"+color1+",#"+color2+","+color3+","+b+")"
				} );
			}
		...
		function Initialize() {
			...
			setInterval( TimeUpdate, 100 );//先清除，再生成 
			
			ResizeCanvas();//设置CAnvas的宽高
		}
